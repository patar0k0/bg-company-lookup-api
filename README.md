...
`{"query", "report", "official_data", "web_context_sources", "priority_assessment",
"addresses"}`. Ако фирмата не е намерена в официалния регистър, `official_data` и
`priority_assessment` са `null`, а докладът се генерира само от уеб частта.
...
`addresses` обединява регистровите адреси (адрес на управление + за кореспонденция) с адреси,
намерени чрез уеб търсене (офиси, обекти и др.), маркирайки уеб адресите, различни от
